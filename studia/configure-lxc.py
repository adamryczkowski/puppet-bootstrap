...
def readFromBashCommand(command, args, sudo=True):
    cmdstr=command + ' ' + ' '.join(args)
    if sudo:
        cmdstr='sudo ' + cmdstr
...
def find_string_in_file(file, search_regexp):
...
if args.internalif != "auto":
    ...
    args.internalif = readFromBashCommand('''augtool -L -A --transform "Shellvars incl /etc/default/lxc-net" get "/files/etc/default/lxc-net/LXC_BRIDGE" | sed -En 's/\/.* = \"?([^\"]*)\"?$/\1/p'`''', [])

if '#LXC_DHCP_CONFILE' in open('/etc/default/lxc-net').read():
	executeBashCommand('sed', ['-',
                               'i',
                               '-',
                               'e',
                               '''"/\#LXC_DHCP_CONFILE/ s/\#//"''',
...
if '#LXC_DOMAIN' in open('/etc/default/lxc-net').read():
